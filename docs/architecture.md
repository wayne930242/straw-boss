...
| `notifying-main-agent` | skill (invoked by a Claude dispatched agent, not the main agent) | Routes a Claude worker to a Claude or Codex main agent by recorded capability: herdr primary for every provider pair, `SendMessage` only as Claude-to-Claude fallback. `report-task-status.py` owns write-before-herdr ordering and `watch-plan-status.py` remains the recoverable scheduling signal |
| `asking-peer-agents` | skill (invoked by a dispatched agent, not the main agent) | The lateral counterpart to `notifying-main-agent`: one dispatched agent checking another's live progress or conclusion, instead of investigating that task's app/worktree directly. Resolves the target from the same `~/.straw-boss/dispatch/` instructions `dispatching-work`'s list branch reads (`repo_root` answers "which worktree is even relevant"), reads its trail via `peeking-work` first, then prompts any recorded Claude or Codex `herdr-pane` directly. `SendMessage` is a Claude-to-Claude fallback only; a `claude-p` peer or a dispatch without `herdr_pane_id` is routed through the main agent instead. |
