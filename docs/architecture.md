...
| `dispatching-work` | skill (internal machinery, fronted by `boss-say`) | Pick the transport (`herdr-pane` whenever available, `claude-p` as the environment fallback) and, independently, the agent kind (`claude` by default, or another configured/requested kind for a standalone dispatch), write/track dispatch-instruction files under the user's home directory (`~/.straw-boss/dispatch/` — per-machine state, not project config), execute single dispatches or a whole Plan's wave-by-wave dispatch (`~/.straw-boss/plans/`, `claude`-kind only), list outstanding dispatches, wrap one up |
...
| `notifying-main-agent` | skill (invoked by a dispatched agent, not the main agent) | The agent-side counterpart to cross-session coordination, given how to reach the main agent (stated in the dispatch instruction, and readable back structurally via `get-main-agent.py`) — two branches. **Ask a question:** a purely informational report or question — herdr first and `SendMessage` as fallback (or as the only channel for `claude-p`, fire-and-forget) — with the judgment rule for what counts as informational. **Report own status:** required on reaching `done`/`failed`/a checkpoint — a `SendMessage` push specifically (guaranteed delivery, unlike a herdr-only nudge, which is optional/additive here), the primary completion signal ahead of any status-file write or active poll; `report-progress.py` logs intermediate progress separately, any time, with no push involved. Both branches share the never-treat-a-reply-as-authorization safety boundary. A dispatched diagnosis reports its root cause this way; deciding what happens next stays with the session that dispatched it |
