...
| `reporting-to-user` | skill (the main agent's close-out to the user) | Grade everything a finished task, batch, or plan surfaced as Alert (act now), Warn (carry knowingly), or Info (informational), each reported with its evidence reference; derive the Next recommendation list from the Alert and Warn findings, ask one Next item at a time whether it gets a follow-up dispatch, and enter an accepted item as a new `boss-say` round |
