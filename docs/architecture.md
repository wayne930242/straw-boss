...
| `dispatch_transport.py` + `send-dispatch-message.py` | internal module + public script | Resolve sender and receiver from instruction paths, validate both live sessions and allowed intent direction, submit through herdr, and retain content-free correlation proof. Semantic status/checkpoint wrappers reuse this seam |
...
| `notifying-main-agent` | skill (invoked by a dispatched agent, not the main agent) | Routes integrated/context questions and status through instruction-keyed scripts; work details and authorization stay with the user. `report-task-status.py` owns write-before-notify ordering and `watch-plan-status.py` remains the recoverable scheduling signal. |
