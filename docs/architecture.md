...
| `dispatching-work` | skill (internal machinery, fronted by `boss-say`) | Pick the transport (`herdr-pane` whenever available, `claude-p` as the environment fallback) and, independently, the agent kind (`claude` by default, or another configured/requested kind), write/track dispatch-instruction files under the user's home directory (`~/.straw-boss/dispatch/` — per-machine state, not project config), execute single dispatches or a mixed-agent Plan's wave-by-wave dispatch (`~/.straw-boss/plans/`), list outstanding dispatches, wrap one up |
| `dispatch-task.py` + `launch-dispatched-agent.py` | public scripts | Generate an immutable per-dispatch contract before launch, inject it at Claude system-prompt or Codex developer-instruction priority, record a launch receipt, and refuse confirmation when receipt, digest, pane, provider, or session identity diverges |
| `dispatch_transport.py` + `send-dispatch-message.py` | internal module + public script | Resolve both communication directions from an instruction path, validate the live receiver session, submit through herdr, and retain content-free delivery proof. Semantic status/checkpoint wrappers reuse this seam |
...
| `notifying-main-agent` | skill (invoked by a dispatched agent, not the main agent) | Routes questions and status through instruction-keyed scripts. `report-task-status.py` owns write-before-notify ordering and `watch-plan-status.py` remains the recoverable scheduling signal. |
| `asking-peer-agents` | skill (invoked by a dispatched agent, not the main agent) | Reads a resolved peer's progress first, then calls `send-dispatch-message.py --to worker`; caller-visible pane, name, provider, and session routing are eliminated. |
| `init` | skill (user-invoked, one-time/occasional setup) | Ask which apps to manage, write `apps.json` and sync root `CLAUDE.md`; ask, once project-wide, whether to enable one or more additional agent kinds and what kind of work/model/effort each should use, recording the answer as a routing-policy section in root `CLAUDE.md`; record whether herdr-backed dispatch is enabled (before the bootstrap check below, so its dispatch decision has real capability info to work with); check each app for a missing agent system and, on confirmation, dispatch `create-great-harness` into it |
...
6. `shipping-task` obtains user authorization for every merge (and any push outside the task's own feature branch) each agent reaches (reported as `awaiting-authorization`, not silence), resuming it to proceed rather than letting it self-authorize; commit, and pushing the task's own feature branch, need none — the latter is reported as a non-blocking FYI instead. It does the same for any tracker ticket tied to the work. A substantive work-content question (`awaiting-user-input`) is different — `shipping-task` only points the user at the task's own pane, it does not relay or resume. A dispatched diagnosis reports root cause through the shared completion-status command, which writes before notifying the main agent's herdr pane, and the session that dispatched it decides whether to hand off to `boss-say` for the fix.
...
Merge, and a push landing outside the task's own feature branch (a monorepo-root submodule pointer-bump, an app-owned git-workflow skill's protected-branch release push), require explicit user authorization, every time — an agent cannot self-authorize either. `shipping-task`'s dispatch instructions tell the agent to stop and report readiness once it's ready for one of these, and `shipping-task` (interactive with the actual user) obtains authorization and resumes it. This holds for both dispatch modes.
