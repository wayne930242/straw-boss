...
The user's job is to hand over work; deciding how it gets done is the main agent's job. `boss-say` is the single door for anything — implementation, audit, research, or diagnosis — and its first act is triage, on two independent axes:
