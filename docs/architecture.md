...
| `reporting-to-user` | skill (the main agent's close-out to the user) | Grade everything a finished task, batch, or plan surfaced as Alert (act now), Warn (carry knowingly), or Info (informational); report Alert first with evidence references, ask one decision at a time whether each Alert and Warn finding gets a follow-up dispatch, and enter an accepted follow-up as a new `boss-say` round |
