...
| Component | Kind | Role |
|---|---|---|
| `work-on` | skill (invoked internally by the entry points and by `shipping-task`) | Classify a request against the project's configured apps (`.claude/straw-boss/apps.json`), apply any configured legacy redirect; for implementation work, decompose into a Plan when it's more than one task (confirmed with the user), then hand off to dispatch |
...
| `shipping-task` | skill (invoked by `boss-say`) | Implementation lifecycle for **one** task, user-picked per task: full flow (worktree → develop → MR → merge → archive) or light flow (direct commit to base) — delegated to the target app's own `gitWorkflowSkill` where configured, else this skill's fallback steps; dispatches via `dispatching-work` and gates every mutation on explicit user authorization |
| `boss-say` | skill (**the** implementation entry point) | Every request to get something built comes here. Triages scale first, then picks the dispatch shape itself: one logical request (even a multi-phase one) goes to `shipping-task`; many independent items become a `dispatching-work` plan with no dependency edges, dispatched under this skill's own concurrency cap (slicing the ready wave instead of firing it all at once) and refilled as items finish — driven either inside one turn (`Monitor`) or, for a batch too big for one turn, by a `/loop` this skill starts itself |
...
| `troubleshooting-app` | skill (entry point) | Diagnose (app-code vs. infra) in-session, hand off to `boss-say` once root cause is known |

`inspecting-app`/`investigating-app` hand off to general-purpose audit/research skills that aren't part of straw-boss — they're expected to already exist in your own Claude Code setup (most setups have something like this). If you don't have equivalents, those two entry points have nothing to hand off to; `boss-say`, `shipping-task`, `troubleshooting-app`, and `work-on` stand on their own.

## Why one entry point for all dispatch

The user's job is to hand over work; deciding how that work gets dispatched is the boss's job. Asking the user to pick the right entry skill — one task or many? one turn or a loop? — pushes an operational decision onto the person least interested in it, and gets it wrong whenever the request's real scale only becomes clear after the items are read. So `boss-say` is the single door for anything implementation-shaped, and its first act is triage:

- **One logical request** — even one that decomposes into phases or spans several apps — is still one unit of work: it goes to `shipping-task`, which owns the flow question and the per-task git lifecycle.
- **Many independent items** become a batch: one plan, no dependency edges, dispatched under a concurrency cap.
- **A batch too large for one turn** self-paces — `boss-say` starts the `/loop` itself rather than handing the user an incantation to type.

The shape is stated, not asked. A user who disagrees overrides it in one sentence, which is cheaper than a question asked on every dispatch.
...
1. `boss-say` fires on any request to get something built; the read-only entry points (`inspecting-app`, `investigating-app`, `troubleshooting-app`) fire on audits, research, and unexplained failures.
2. Read-only entry points invoke `work-on` to classify the request against the configured apps (applying any legacy redirect), then continue directly in the current session against the resolved app's real files — no dispatch. `troubleshooting-app` hands its root cause back to `boss-say` when a fix is wanted.
3. `boss-say` triages scale first and states the shape it picked: one logical request → `shipping-task`; many independent items → its own capped batch, run in this turn or under a `/loop` it starts itself. Batch items resolve their apps via `work-on` here; a single request resolves its app inside `shipping-task`.
...
5. `shipping-task` (or `boss-say` for a batch item) assembles each task's description — pointing at an existing OpenSpec change by name where Task 4 found and confirmed one, never restating its scope — and invokes `dispatching-work` (single instruction, or the whole Plan), which picks a mode per task, writes instruction(s), and executes them in sessions rooted in the target app(s).
