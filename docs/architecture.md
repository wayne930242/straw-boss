...
| `troubleshooting-app` | skill (a specialist `boss-say` drives) | Classify from supplied evidence, dispatch app diagnosis with evidence obligations, then hand the fix back to `boss-say` once root cause is known |
