...
| `notifying-main-agent` | skill (invoked by a Claude dispatched agent, not the main agent) | The Claude-side fast notification and informational-question channel. Every Plan task, including Codex, separately persists status through `report-task-status.py`; `watch-plan-status.py` is the provider-neutral scheduling signal. Claude `SendMessage` reports remain an additive low-latency path and carry the same never-treat-a-reply-as-authorization boundary |
| `asking-peer-agents` | skill (invoked by a dispatched agent, not the main agent) | The lateral counterpart to `notifying-main-agent`: one dispatched agent checking another's live progress or conclusion, instead of investigating that task's app/worktree directly. Resolves the target from the same `~/.straw-boss/dispatch/` instructions `dispatching-work`'s list branch reads (`repo_root` answers "which worktree is even relevant"), reads its trail via `peeking-work` first, and only reaches out via `SendMessage` for a peer that's actually addressable — `herdr-pane`+`claude` with a recorded `herdr_pane_id`, looked up live via `herdr agent get` rather than guessed at. A `claude-p`/`codex` peer, or one with no `herdr_pane_id` yet, isn't reliably addressable this way, so the question goes to the main agent instead |
