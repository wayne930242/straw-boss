...
- Local automated tests: `uv run --with pytest pytest -q tests` -- 576 passed, 238 subtests passed, exit 0 (includes the 11 new tests in `tests/test_codex_renewal_usage.py` and the full pre-existing `test_context_renewal.py` / `test_jev_codex_renewal.py` / `test_jev_codex.py` suites, unchanged).
