...
export const STATE_CONTEXT =
  'A coding assistant history is being compacted. `history` runs oldest first, with tool results omitted and long text abridged. Score the named call and its full result for the unfinished task. Keep governing instructions and required evidence; archived originals are recoverable, but some tool actions cannot be reproduced.';
...
export function collectToolCalls(
  ...
  messages.forEach((message, callIndex) => {
    for (const tool of message.toolUses) {
      ...
      if (!found) continue;
      const governingSource = protectedInputPatterns.some((pattern) => new RegExp(pattern).test(JSON.stringify(tool.input)));
      calls.push({
        id: `t${calls.length + 1}`,
        ...
        ...(governingSource ? { governingSource: true } : {}),
        pinned:
          governingSource || isPinned(callIndex, messages.length, preserveRecentMessages) ||
