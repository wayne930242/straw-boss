...
export function resolveOptions(options: CompactOptions = {}): ResolvedCompactOptions {
  return {
    ...
    ...(options.protectedInputPatterns ? { protectedInputPatterns: options.protectedInputPatterns } : {}),
...
export function questionsFor(call: ToolCall, criteria?: RetentionCriteria): JevQuestions {
  if (criteria) {
    return Object.fromEntries((["keepCall", "keepResult"] as const).map((kind) => [
      `${kind === "keepCall" ? "call" : "result"}_${call.id}`,
      { type: "noul" as const, instructions: `Target: tool call ${call.id} (${call.tool}, ${call.resultChars} result chars). ${criteria[kind]}`,
        criteria: { true: criteria[kind], false: "The proposition is false; this material can be pruned according to the stated retention contract." } },
    ]));
  }
  return {
    [`call_${call.id}`]: {
...
export function batchCalls(
  ...
  stateTokens: number,
  options: Pick<ResolvedCompactOptions, 'maxRequestTokens' | 'criteria'>,
): ToolCall[][] {
  ...
  for (const call of calls) {
    const tokens = estimateTokens(JSON.stringify(questionsFor(call, options.criteria)));
...
export function decideCall(
  call: Pick<ToolCall, 'id' | 'tool' | 'pinned' | 'governingSource'>,
  answer: CallAnswer,
  options: Pick<ResolvedCompactOptions, 'keepThreshold'>,
): CallDecision {
  const base = { id: call.id, tool: call.tool, ...answer };
  if (call.pinned) return { ...base, action: 'keep', reason: call.governingSource ? 'governing_source' : 'pinned' };
...
async function askBatch(
  ...
  const questions: JevQuestions = Object.assign({}, ...batch.map((call) => questionsFor(call, criteria)));
...
function truncatedResultText(text: string, isError: boolean, headChars: number): string {
  ...
  return `${head}[fast-jev-compaction truncated ${text.length - headChars} chars of this tool result${
    isError ? ' (error)' : ''
  }; original saved in the recovery record]`;
}
...
export function applyDecisions(
  ...
  for (const message of messages) {
    ...
    if (message.text.length === 0 && toolUses.length === 0 && toolResults.length === 0) {
      continue;
    }
...
export async function compact(
  ...
  const calls = collectToolCalls(messages, resolved.preserveRecentMessages, resolved.protectedInputPatterns);
  ...
  if (candidates.length > 0) {
    ...
    const settled = await Promise.allSettled(
      batches.map((batch) => askBatch(asker, state.state, batch, resolved.criteria)),
    );
    const failed = settled.find((item) => item.status === "rejected");
    if (failed?.status === "rejected") throw failed.reason;
    for (const item of settled) if (item.status === "fulfilled") {
      for (const [id, answer] of item.value) answers.set(id, answer);
    }
  }
  ...
  return {
    ...
    stats: {
      ...
      pinned: count(decisions, 'pinned') + count(decisions, 'governing_source'),
