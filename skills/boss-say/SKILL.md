...
7. **A feature-branch push notification is not a plan-status event at all — it never appears in `read-plan-status.py`/`Monitor`, only as a raw `SendMessage`.** A full-flow task pushed its own feature branch and opened or updated an MR/PR on its own (`shipping-task`'s Task 3/Overview, unchanged for a batch item) — it needed no authorization and was never waiting. On the `SendMessage` push reporting it (`notifying-main-agent`'s "Branch: Report a completed push, then continue"), relay it to the user as an FYI (branch + MR/PR reference) and move on — don't treat it as `awaiting-authorization`, don't attempt to obtain authorization for it, and it never holds or frees an in-flight slot, since the task's own plan status never changed. Unlike every status this step list otherwise reacts to, there is no `Monitor` fallback for this one — a missed `SendMessage` here has no polling backstop, the same accepted limitation the informational-question channel already has.
...
**One-shot batch:** drive the batch as far as it goes within this turn. Every in-flight task also sends a `SendMessage` push the moment it reaches `done`/`failed`/a checkpoint (per `notifying-main-agent`'s "Branch: Report your own status") — a faster signal than the polling loop below, but not a substitute for running it: use a real `Monitor` polling loop over the plan's `status/` directory (exact command in `plan-mechanics.md`) as the fallback that detects `done`/`failed`/`cancelled`/`awaiting-authorization`/`awaiting-user-input`/`awaiting-main-agent` events even for a task whose push didn't land, and react per Task 5 — including resolving any `awaiting-main-agent` finding inline (step 6) before continuing. A full-flow task's feature-branch push notification is separate from all of this — it arrives only as its own `SendMessage` (`notifying-main-agent`'s "Branch: Report a completed push, then continue"), never through the status-file/`Monitor` path, so relay it per Task 5 step 7 whenever it lands, not on any polling cadence. Continue until either every task is terminal, or the turn has to stop and hand something back to the user (an authorization or a question, including a fully stalled batch per Task 5 step 8). **Never call `ScheduleWakeup` in this shape** — there is no `/loop` iteration to schedule.
...
- "Still idle, peek it again to be safe" on every Monitor notification or `/loop` tick where nothing about the task actually changed — no, Task 5 step 8: peek once per stretch of idleness and repeat the prior finding for an unchanged state; re-peeking on every notification is an unbounded loop, not thoroughness.
