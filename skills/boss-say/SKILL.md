...
**Whether the dispatch machinery still applies depends on where the named skill's work actually lands, not on how it was invoked.** A skill whose work stays outside any managed app's checkout (a tracker/API-driven skill, a pure lookup, a doc-writing skill) runs solo, right here — this branch's hand-off is exactly that, no app-dir rooting, `dispatching-work` never invoked. A skill whose work lands inside an app's own checkout (edits code, runs its tests) still needs Task 1's execution-tier call first — resolve the app via `work-on`, then decide solo-vs-dispatched before invoking it, the same as any other item; naming the skill explicitly doesn't skip that gate.
