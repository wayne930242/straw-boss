...
4. On a `done`/`failed`/`cancelled` status event for any in-flight task: auto-detach it (same rules as `dispatching-work`'s plan branch — close its pane/tab if `herdr-pane`, remove its worktree if full-flow, call `wrap-up-task.py`), then repeat from step 1 — a freed slot gets backfilled from the queue immediately, not batched up for later. `cancelled` is the one status this skill may write itself, per `docs/roles.md`'s autonomy boundary — when it does, run this same step inline; the watcher will also emit the persisted revision for recovery by a resumed session.
...
7. **A feature-branch push notification is not a plan-status event — it never appears in `read-plan-status.py` or `watch-plan-status.py`.** A full-flow task pushed its own feature branch and opened or updated an MR/PR on its own (`shipping-task`'s Task 3/Overview, unchanged for a batch item) — it needed no authorization and was never waiting. Relay the provider's fast report to the user as an FYI (Claude: `SendMessage`; Codex: live output or herdr pane), then move on: don't treat it as `awaiting-authorization`, don't attempt to obtain authorization for it, and don't change slot accounting. The durable watcher intentionally covers scheduling statuses only, so this informational event has no status-file backstop.
...
**One-shot batch:** drive the batch as far as it goes within this turn. Run `watch-plan-status.py --plan <slug>` as the authoritative scheduling stream and react to every persisted `done`/`failed`/`cancelled`/checkpoint revision per Task 5. Claude tasks additionally send `SendMessage` as a low-latency hint; Codex tasks surface progress through live output or their herdr pane. Those provider-specific channels accelerate observation but never replace the watcher. Feature-branch push FYIs remain outside Plan status and are relayed per Task 5 step 7 whenever observed. Continue until either every task is terminal, or the turn has to stop and hand something back to the user (an authorization or a question, including a fully stalled batch per Task 5 step 8). **Never call `ScheduleWakeup` in this shape** — there is no `/loop` iteration to schedule.
