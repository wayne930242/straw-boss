...
**Commit needs no authorization — the agent commits on its own as it goes. Neither does pushing the task's own feature branch** (opening or updating an MR/PR against it) — the branch was already implicitly authorized when the main agent created it for this task; the agent reports through the recorded main-agent herdr pane and continues, using `SendMessage` only for Claude-to-Claude fallback or a progress record when no live route exists. **Merge is the mutation the agent cannot self-authorize** — as is any push that lands on another tracked branch: every instruction tells the agent to stop and persist `awaiting-authorization`, not execute it.
...
- **Full flow fallback**: develop inside the main-agent-created worktree, committing to the feature branch freely, then push the branch and open an MR/PR on its own. Report the branch + MR/PR reference through the recorded main-agent herdr pane without waiting; use `SendMessage` only for Claude-to-Claude fallback or `report-progress.py` when no live route exists. Continue; only stop before merge or another-branch push.
...
Build the task description for `dispatching-work` with the requested outcome, app development/SDD route, selected lifecycle/worktree, mutation gates, tracker boundary, checkpoints, and shared-resource command. Every kind receives exact `report-progress.py` and `report-task-status.py --instruction-path <path>` commands. The status command writes durable state before prompting the recorded main-agent herdr pane; a Plan watcher remains the scheduling recovery path. Claude instructions point to `notifying-main-agent` for routing and explicitly prohibit `SendMessage` unless the main agent is also Claude. Codex instructions inline the same herdr/status behavior without citing Claude skills. Pass both provider kinds and validated reachability to `dispatch-task.py write`.
