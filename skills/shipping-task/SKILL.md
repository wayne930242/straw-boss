...
Build the task description for `dispatching-work` with the requested outcome, app development/SDD route, selected lifecycle/worktree, mutation gates, tracker boundary, checkpoints, and shared-resource command. The generated contract supplies exact progress, message, and status commands for every provider. Pass both provider kinds and the validated main pane/session pair to `dispatch-task.py write`.
