...
See `docs/roles.md` for the cast of characters and the authority framework (including the push/merge authorization gate below) this skill operates under — not redefined here.
...
**Commit needs no authorization — the agent commits on its own as it goes.** Push (opening an MR/PR) and merge are the mutations the agent cannot self-authorize: every dispatch instruction this skill assembles explicitly tells the agent to stop and report back once it's ready to push or merge, not execute it. This skill (interactive with the actual user) obtains authorization and resumes the agent to proceed. This holds for both dispatch modes and regardless of which app owns its own git-workflow skill. On the light flow there is no push/MR/merge step at all — commit straight to the base branch is the only mutation, and it needs no authorization either; `forbidDirectCommit` (Task 2) is the only remaining gate on that path.
...
When the agent reports it's ready to push an MR or merge, state exactly what's about to happen (branch + target base for a push/MR; MR + target branch + confirmed outcome so far for a merge) and get explicit authorization from the user — every time, even after a prior authorization earlier in the same task. Then resume the agent with that authorization (`claude -p --resume <session_id> "..."` for `claude-p`, `herdr agent prompt <name> "..." --wait` for `herdr-pane`) to actually execute it. Repeat for each mutation checkpoint the agent reaches (push/MR, then later merge).
...
If the target app is itself a submodule of a monorepo root and a pointer-bump push at the root is also needed once its commit lands, that's a separate mutation — ask about it separately, don't fold it into the same authorization.

**Verification:** every push/merge the agent performed was preceded by an explicit authorization in this conversation, obtained by this skill, not assumed or granted by the agent itself — commit itself needs none, on either flow.
...
- "Commit doesn't need authorization anymore, so push/MR probably doesn't either" — no, only commit is unauthorization-gated; push/MR and merge still need it, every time, per Task 5.
...
- "The agent can just ask the user directly since it's an interactive herdr pane" — for a push/merge *authorization*, no — Task 5 still routes that through this skill regardless of mode (commit itself needs no authorization, so there's nothing to route for it). For a substantive work-content *question* (`awaiting-user-input`), yes — that one is meant to go directly to the user in the task's own pane; don't conflate the two checkpoints.
