...
3. **Pipe-test it** with a synthesized stdin payload matching the real hook-input shape (`{"tool_name":"Bash","tool_input":{"command":"<the exact destructive command this should block>"}}`) piped directly into the constructed command. Confirm it actually blocks. Also pipe-test one command it should **not** block (e.g. a plain `git push`), to confirm it isn't over-matching.
