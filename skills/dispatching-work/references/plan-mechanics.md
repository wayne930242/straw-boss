...
**Escalation order before reaching for `awaiting-user-input`.** Not every difficulty is a judgment call for the user. Three distinct cases, in order:
1. **Missing context the main agent already has** (another task's status, which apps are in scope) — that's `notifying-main-agent`'s informational-question branch, not this checkpoint at all.
2. **Genuine technical difficulty** — stuck on how to solve or debug something, not missing context and not a values/architecture call — try a stronger second opinion first, if one is available to the task (e.g. this session's own `advisor` tool, when present), before escalating further. Don't assume a specific tool is available; if none is, go straight to step 3.
3. **A judgment call reserved for the user** (which of several valid approaches, how to interpret an ambiguous requirement) — or genuine technical difficulty a second opinion didn't resolve — this is `awaiting-user-input`, as described above.

A second opinion is consultative, never decisive on the user's behalf — it can help the task get unstuck on *how*, it cannot make a call that's inherently the user's to make. It also never substitutes for the informational-question branch (step 1) or for the authorization flow below, which is unaffected by any of this.
