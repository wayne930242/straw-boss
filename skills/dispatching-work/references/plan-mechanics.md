...
`awaiting-authorization`, `awaiting-user-input`, and `awaiting-main-agent` are all not terminal — the task's own `plan.json` entry stays `dispatched`, none joins or leaves the ready wave. All three exist so `Monitor` can detect the checkpoint the same way it detects `done`/`failed`, instead of a task sitting silently idle with no signal that it's actually waiting on someone.

## Authorization checkpoints (full flow only)

An agent must stop and report readiness rather than execute a merge, or a push landing outside its own feature branch (a monorepo-root submodule pointer-bump, an app-owned git-workflow skill's protected-branch release push) — the main agent (in practice, `shipping-task`, not `dispatching-work` itself) obtains authorization and resumes it. Commit, and a push of the task's own feature branch, need no authorization and reach no checkpoint here — a feature-branch push is instead reported as a non-blocking `SendMessage` FYI (`shipping-task`'s Task 5), never this status. Within a plan, that checkpoint is reported the same way completion is: the dispatch instruction for every full-flow task MUST tell the agent to call the status script with `--status awaiting-authorization` (not just stop silently) the moment it's ready to merge or to push outside its own feature branch, before it actually stops. This is what makes the checkpoint visible to `Monitor` — without it, a task waiting on authorization looks identical to a task still working, which is exactly the "silence is not success" failure `Monitor`'s coverage requirement exists to prevent.
...
Different from an authorization checkpoint on every axis that matters: it isn't a mutation gate, and the main agent doesn't act as an intermediary. A dispatched task's instruction MUST also tell it: if it hits a substantive question about the *work itself* — which of several valid approaches to take, how to interpret an ambiguous requirement, whether an existing OpenSpec change it found mid-task should be extended or left alone — that isn't a merge/other-branch-push decision, it calls the status script with `--status awaiting-user-input` and the question in `--note`, then asks the question directly in its own pane and waits there. The user can answer it directly in that pane — the main agent does not need to relay the question or the answer, and should not try to guess the answer on the task's behalf.
...
1. **Missing context the main agent already has, that doesn't block continued progress while waiting** (another task's status, which apps are in scope) — that's `notifying-main-agent`'s informational-question branch, not either checkpoint below.
...
**Not every mid-task question needs the user.** When a task's question is something the main agent can answer directly from what it already knows (another task's status, which apps are in scope) — not a judgment call about the work — it uses `SendMessage` to the main agent instead of `awaiting-user-input`, and doesn't touch the status file at all for that exchange. See `references/cross-session-coordination.md` for addressing and the safety boundary (a peer's reply is never authorization for anything).
...
## Main-agent-action checkpoints (`herdr-pane`, `agent_kind: claude` only)

For being blocked on an action only the main agent's own judgment or dispatch authority can take (redispatching a failed dependency, arbitrating a conflict with a peer task) — not a mutation gate, not a question. A dispatched task's instruction MUST state this checkpoint exists, same as it must for `awaiting-user-input` above: `--status awaiting-main-agent`, what's needed in `--note`, then wait.
...
**This write is bookkeeping, not a notification.** It's what `plan.json` sync and wave computation read — it does not by itself tell the main agent anything happened (see "Monitoring completion" below for why that distinction matters now). Every dispatch instruction for a plan task also MUST tell the agent to send the `SendMessage` push required by `skills/notifying-main-agent/SKILL.md`'s "Branch: Report your own status," immediately after this write — same requirement a standalone dispatch has, no plan-specific exception. The task MAY also call `report-progress.py --instruction-path <path> --note "<text>"` at any point beforehand to log intermediate progress — a separate, non-notifying, any-time channel (same reference doc).
...
4. Every dispatch instruction for a plan task explicitly states: (a) run the status-reporting script on completion/failure, **and** send the `SendMessage` push required by `notifying-main-agent`'s "Branch: Report your own status" — the status write alone is not sufficient (see "Reporting status" above), (b) never touch any tracker ticket, (c) if worktree-backed, the shared-resource-coordination text (see below — a caveat at minimum, an actual `claim-port`/`wait` command if the task will run a dev server or touch a shared DB) and that the worktree already exists at the given path, (d) if another task in the plan depends on this one, the exact `artifacts/` path (see "Cross-task artifacts" above) to write its real output to — and if this task depends on another, the exact `artifacts/` path to read that prerequisite's output from, stated as required input, not optional context, (e) for `herdr-pane` tasks, that a substantive work-content question (not a git mutation) gets reported via `awaiting-user-input` and asked directly in its own pane, per "User-clarification checkpoints" above (including the escalation order before reaching it) — never guessed at or silently deferred, (f) that checking another task's live progress or conclusion goes through the `asking-peer-agents` skill rather than investigating that task's app/worktree directly — it also states this task's own name (below), which is what makes it reachable as someone else's peer in turn, (g) for `herdr-pane` tasks under `agent_kind: claude`, that being blocked on an action only the main agent's own judgment or dispatch authority can take (not a question) gets reported via `awaiting-main-agent`, per "Main-agent-action checkpoints" above (including the escalation order before reaching it) — never routed through the fire-and-forget informational-question channel instead, since that carries no expectation of timeliness, (h) for every full-flow task, the fetch+rebase-before-push step per "Rebase before push (parallel sibling tasks against the same base)" below — always include it, not only when the wave happens to have more than one full-flow task, since a single-task wave can still land behind a later wave's merges by the time it's ready to push.

## Monitoring completion (fallback — the `SendMessage` push is primary)

**This polling loop is a fallback for a plan that's gone quiet without a push arriving, not the primary way the main agent learns of a task's outcome.** Confirmed live: a plan task's status file reported `done`, correctly, with no confirmation the orchestrating main agent's own `Monitor` loop was still alive to notice it — a status-file write is bookkeeping, not proof anyone was told (see "Reporting status" above). The `SendMessage` push required by `notifying-main-agent`'s "Branch: Report your own status" is what actually notifies; this loop exists for when a `Monitor` genuinely wasn't running, was restarted, or otherwise missed the push, and stays useful for wave-scheduling reads (`read-plan-status.py --ready`/`--in-flight`) regardless.
...
Use the `Monitor` tool with a command like the above (a real polling loop, not a one-shot check) — `description` should name the plan. The loop must emit a line for **every** status a task can report — `done`, `failed`, `cancelled`, `awaiting-authorization`, `awaiting-user-input`, and `awaiting-main-agent` — not just `done`; a filter that only matches `done` goes silent on a stuck/crashed task and on one quietly waiting on someone, all of which read identically to "still running" (confirmed live for `done`/`failed`: both produced their own line in the same notification when it's the *first* status a task's file ever reports — the polling loop above dedups on filename via `seen`, so it emits once per file, not once per write). On a `done`/`failed` notification, recompute the ready wave (step 1 above) and dispatch anything newly unblocked. On an `awaiting-authorization` notification, hand it to whichever caller owns that task's authorization gate (typically `shipping-task`) — do not treat it as done, failed, or ready-for-a-new-wave. On an `awaiting-user-input` notification, tell the user which pane to go answer it in and otherwise leave the task alone — same non-terminal treatment, but no relaying, no resuming; the user's own reply in that pane is what un-blocks it. On an `awaiting-main-agent` notification, resolve it directly with `reply-to-worker.py` (see "Main-agent-action checkpoints" above) — same non-terminal treatment as the other two, but no relaying to the user at all; the main agent is the one who un-blocks it.

**`cancelled` does not reliably produce its own Monitor notification, and the main agent must not wait for one.** Cancel (`cross-session-coordination.md`) writes the status file itself — if the task had no prior status file, that write is the file's first appearance and the loop's `seen`-by-filename dedup lets it through like any other first write; but cancelling a task that already reported `awaiting-authorization`/`awaiting-user-input`/`awaiting-main-agent` overwrites that *same* filename, which `seen` has already recorded, so the loop never re-emits it. This isn't something to fix here — the main agent performing a cancel already knows it happened, synchronously, because it did the write; it runs its own cleanup and wrap-up inline as part of Cancel (see "Auto-detach on terminal state" below) rather than waiting on a Monitor line that may never come.
...
Worktree isolation covers files, not a fixed network port a dev server binds to or a database multiple main agents' tasks might verify migrations against — both live outside any one checkout, and outside any one main agent's own visibility (a main agent has no idea what another, independently running main agent has dispatched). Every worktree-backed dispatch instruction includes, at minimum, this line verbatim (or equivalent): *"This worktree is isolated from other worktrees and the shared dev environment. If you run a local dev server to verify your changes, its default port may collide with another worktree's or the shared environment's port or hot-reload connection — check before assuming a bind failure means something else is wrong."*

If the task will actually run a local dev server or touch a shared (non-per-worktree) database for migration verification, go further — see `references/shared-resource-coordination.md` and put the exact `claim-resource.py claim-port` (flexible port) or `claim-resource.py wait` (fixed port or DB migration) command into the instruction, with `--requester-boss` set to the same main-agent-reachability value the instruction already carries for `notifying-main-agent` (`herdr-pane` gets its own pane id, both modes get the `SendMessage` peer name). No port is allocated or reassigned automatically beyond what that reference documents.
