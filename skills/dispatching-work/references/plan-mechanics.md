...
```
Every dispatch instruction for a plan task MUST tell the agent to run this on completion or failure — this is how the main agent finds out, not by asking the agent directly. The script writes only `status/<task-id>.json` — it must never touch `plan.json` or another task's status file.
