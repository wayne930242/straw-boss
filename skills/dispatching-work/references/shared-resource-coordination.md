...
**Releasing a dispatch-time claim — every terminal status, every path.** The worker never claimed this lock, so nothing it does releases it and it has nothing to report about it. Whoever wraps the instruction up releases it: `dispatching-work`'s Wrap-up branch for a standalone dispatch, the same skill's plan auto-detach for a batch item or a plan task. `done` is not an exception.
