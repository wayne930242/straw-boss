...
**When no `--name` flag was found, the renamed peer name MUST be unique to this main-agent session, never the bare `straw-boss-orchestrator` literal.** Two concurrent main agents (a scenario `shared-resource-coordination.md` already assumes can happen — "another *main agent's* task can also hit the same port or database") both renaming to the identical literal makes that name resolve to more than one live session. Per `SendMessage`'s own contract, a bare name matching more than one live session doesn't deliver directly — and this project has already recorded a wrong-target `SendMessage` being "reported 'sent' and never seen again, no error anywhere." Now that a dispatched agent's completion push (`notifying-main-agent`'s "Branch: Report your own status") depends on this delivery being real, a shared name isn't just a cosmetic risk — derive a per-session-unique value instead, e.g. `straw-boss-orchestrator-<sanitized $HERDR_PANE_ID>` (`wF:p9` → `straw-boss-orchestrator-wF-p9`).
...
```bash
herdr agent prompt "$HERDR_PANE_ID" "/rename straw-boss-orchestrator-<sanitized \$HERDR_PANE_ID>"
```
...
- `herdr-pane`: *"Your own dispatch instruction file is at `<resolved instruction path>`. Your main agent's herdr pane id is `<resolved $HERDR_PANE_ID value>` and its `SendMessage` peer name is `<the resolved peer name>` — use the `notifying-main-agent` skill if you need to reach it, and to report once you reach `done`/`failed`/a checkpoint (required, not optional — see that skill's 'Branch: Report your own status')."*
- `claude-p`: *"Your own dispatch instruction file is at `<resolved instruction path>`. Your main agent's `SendMessage` peer name is `<the resolved peer name>` — use the `notifying-main-agent` skill if you need to reach it, and to report once you reach `done`/`failed`/a checkpoint (required, not optional), but you cannot wait for a reply before you exit."*

Pass the same `$HERDR_PANE_ID` value (for `herdr-pane`) to `dispatch-task.py write`'s `--main-agent-pane-id`, and the resolved value (detected `--name`, or the `/rename`d value if none was found) to its `--main-agent-peer-name`, when assembling the instruction (`dispatch-mechanics.md`'s "Instruction file") — this is what lets the dispatched agent read reachability back structurally via `get-main-agent.py`, instead of only from the prose above. `--main-agent-peer-name` is required — `dispatch-task.py write` refuses outright if it's omitted, since there is no safe default whenever more than one main agent might be running; always resolve one (the `--name` detection above, or `/rename` if that came back empty) before calling it. That's the whole instruction either way; `notifying-main-agent` itself carries the judgment rule, the channel-selection logic, the reporting requirement, and the never-authorization safety boundary, so none of it needs restating here.
