...
- `herdr-pane`: *"Your own dispatch instruction file is at `<resolved instruction path>`. Your main agent's herdr pane id is `<resolved $HERDR_PANE_ID value>` and its `SendMessage` peer name is `<the unique renamed value>` — use the `notifying-main-agent` skill if you need to reach it, and to report once you reach `done`/`failed`/a checkpoint (required, not optional — see that skill's 'Branch: Report your own status')."*
- `claude-p`: *"Your own dispatch instruction file is at `<resolved instruction path>`. Your main agent's `SendMessage` peer name is `<the unique renamed value>` — use the `notifying-main-agent` skill if you need to reach it, and to report once you reach `done`/`failed`/a checkpoint (required, not optional), but you cannot wait for a reply before you exit."*
