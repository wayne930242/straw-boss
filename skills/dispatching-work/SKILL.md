...
**Five checkpoint/report types — never conflate them:**
...
A task unsure which applies walks `plan-mechanics.md`'s "Escalation order for a stuck task" (full order there, not restated here). `awaiting-authorization` sits outside that order — it's the push/merge readiness gate, not a response to being stuck. All three `awaiting-*` checkpoints are `herdr-pane`, `agent_kind: claude` only; anything else reports `failed` with the situation in `--note`. The report-push row is unconditional — every dispatch sends it on reaching `done`/`failed`/a checkpoint, regardless of mode.
