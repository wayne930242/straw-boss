...
For each app the user said yes to: assemble the task description for `dispatching-work` — `create-great-harness`'s own scope for `<app-dir>`, stated as already confirmed so its Task 1 doesn't re-ask, plus the full provider-and-mode instruction content `cross-session-coordination.md`'s "What the dispatch instruction states, per mode" specifies. Include this dispatch's instruction path so `create-great-harness` Task 6 can call `report-task-status.py --instruction-path`. For a Claude dispatch, also pass the per-session-unique `--main-agent-peer-name`; for Codex, omit that Claude-only option and use the explicit status/progress contract. If more than one app needs bootstrapping this run, share one batch label (`dispatching-work`'s Task 2) across them. Dispatch each through `dispatching-work`'s Tasks 1-5 — never its "Branch: Dispatch a plan," since there's no dependency graph here, just independent single-app dispatches. Track each with `TaskCreate` (one per app, mirroring `shipping-task`'s own Task Initialization) — this spans until each dispatch reports back, not just the ask.

**Detect completion from durable instruction status plus the dispatch transport.** Every provider calls `report-task-status.py` when it finishes or fails. Claude additionally uses `notifying-main-agent`'s `SendMessage` as a low-latency hint; Codex reports through its process output or herdr pane. Confirm the durable status against the mechanism each mode gives you:
