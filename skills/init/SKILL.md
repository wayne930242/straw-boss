...
**Detect completion from durable instruction status plus the dispatch transport.** Every provider calls `report-task-status.py --instruction-path`, which writes status before prompting the recorded main-agent herdr pane. `SendMessage` is only a Claude-to-Claude fallback; otherwise confirm durable status against process/pane observation:
