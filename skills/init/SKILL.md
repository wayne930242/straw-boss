...
For each app the user said yes to: assemble the task description for `dispatching-work` — `create-great-harness`'s own scope for `<app-dir>`, stated as already confirmed so its Task 1 doesn't re-ask, plus the full per-mode instruction content `cross-session-coordination.md`'s "What the dispatch instruction states, per mode" specifies (not just "the reachability pointer" — that block now also includes this dispatch's own instruction path, which `create-great-harness` Task 6 needs to call `report-task-status.py --instruction-path`) both for anything genuinely informational it needs to ask mid-task and for the required completion push (see below — this is the primary way completion gets detected here, not a side channel). Pass `--main-agent-peer-name` (the per-session-unique renamed value, not the bare `straw-boss-orchestrator` fallback) to `dispatch-task.py write` the same as any other dispatch — these bootstrap dispatches are not exempt. If more than one app needs bootstrapping this run, share one batch label (`dispatching-work`'s Task 2) across them. Dispatch each through `dispatching-work`'s Tasks 1-5 — never its "Branch: Dispatch a plan," since there's no dependency graph here, just independent single-app dispatches. Track each with `TaskCreate` (one per app, mirroring `shipping-task`'s own Task Initialization) — this spans until each dispatch reports back, not just the ask.

**Detect completion from the agent's `SendMessage` push — primary — with an active check as the fallback for a bootstrap that's gone quiet.** `notifying-main-agent`'s "Branch: Report your own status" requires every dispatched agent to push a `SendMessage` report the moment it finishes or fails, using `SendMessage` specifically because its delivery is actually guaranteed (per `dispatch-mechanics.md`'s "Reporting scripts" and `notifying-main-agent` itself) — that's the primary signal here, not a courtesy message to wait on indefinitely or ignore. Confirm it against the mechanism each mode also gives you, used as the fallback:

- **`claude-p`:** run each dispatch in the background (`run_in_background`) so independent apps' bootstraps run concurrently rather than serializing this turn; the harness's own background-task notification, once it fires, carries the process's final output — that's a reliable report on its own regardless of whether the push also landed (a `claude-p` process's own exit is already dependable here).
- **`herdr-pane`:** complete `dispatching-work`'s full Tasks 1-5 for that app first — through step 6.5's delivery confirmation, the session-id cross-check, and step 8's flip to `in-progress` — before relying on either signal. Don't start a fallback wait right after step 6's submission: step 6.5 exists specifically because a first-run interruption can swallow the submitted text while `herdr agent prompt --wait` still reports success, and a wait started before that's ruled out can catch the agent still idle on an unanswered prompt and misread it as a finished, empty bootstrap. Once Task 4 is genuinely done for that app, expect its `SendMessage` push; if a bootstrap has gone idle for a while without one landing, fall back to `herdr agent wait "<name>" --until idle --until blocked --timeout 120000` (repeated-`--until` form — see `dispatch-mechanics.md`'s note on this) per app as a background `Bash` call, so multiple apps' waits still run concurrently. A `blocked` return is not completion — it means the agent stalled on a prompt mid-task; surface that to the user rather than wrapping it up as done. On `idle`, `herdr agent read "<name>" --lines 40` to pull the actual report (`create-great-harness`'s own Task 6 output) if the push itself didn't already carry enough detail. On timeout, treat it the same as "hasn't reported back yet" below — still running, not failed.
...
**Verification:** every app from Task 2 was checked against the exemption condition above, not just "does `.claude/` exist"; an app that already met it was never asked; an app that didn't got an explicit yes/skip answer before this task moved on; every "yes" was dispatched (never run inline in this session), with the confirmation carried into the instruction rather than re-asked; for `herdr-pane`, the fallback wait (if needed at all) was started only after that app's own dispatch reached step 8 (delivered confirmed, `in-progress`), never right after submission; a `blocked` wait result was surfaced, never wrapped up as done; completion was detected primarily via the dispatched agent's `SendMessage` push, with the process/background notification (`claude-p`) or `agent wait` + `agent read` (`herdr-pane`) as the fallback for a bootstrap that's gone quiet without one — never assumed complete purely from silence in either direction; every bootstrap dispatch this run started is either wrapped up or explicitly flagged as still running before this run of `init` ends.
...
- "Skip waiting for the dispatched agent's `notifying-main-agent` push, go straight to `agent wait`/`agent read` every time" — no, Task 9: the `SendMessage` push is the primary signal (its delivery is actually guaranteed, unlike a herdr-only nudge); `agent wait`/`agent read` (`herdr-pane`) or the background notification (`claude-p`) is the fallback for a bootstrap that's gone quiet without one, not the default path for every dispatch.
- "The bootstrap's own status file (`.status.json`) says done, that's enough, don't check for the push" — no, per `notifying-main-agent`'s "Branch: Report your own status": a status write is bookkeeping, not a notification; the push (or, absent that within a reasonable wait, the fallback check) is what actually tells you.
