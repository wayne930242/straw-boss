...
class CodexPlanOrchestrationTests(unittest.TestCase):
    ...
    def test_unavailable_herdr_keeps_dispatch_pending(self) -> None:
        instruction_path = self.dispatch("codex")
        instruction = json.loads(instruction_path.read_text())
        ...
        bin_dir.mkdir()
        herdr = bin_dir / "herdr"
        herdr.write_text("#!/bin/sh\necho 'Herdr unavailable' >&2\nexit 1\n")
        herdr.chmod(0o755)
        result = self.run_script(
            "launch-dispatched-agent.py", "--instruction-path", str(instruction_path),
            extra_env={"PATH": str(bin_dir)},
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Herdr unavailable", result.stderr)
        self.assertEqual(json.loads(instruction_path.read_text())["status"], "pending")
        self.assertFalse(instruction_path.with_suffix(".launch.json").exists())

    def test_retired_mode_is_rejected_before_writing(self) -> None:
        result = self.run_script(
            "dispatch-task.py", "write", "--app", "api", "--slug", "retired",
            "--task", "task", "--mode", "claude-p", "--repo-root", str(ROOT),
            "--main-agent-kind", "codex",
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("herdr-pane", result.stderr)
        self.assertEqual(list((self.home / ".straw-boss/dispatch").glob("*.json")), [])
