...
class DispatchedAgentLifecycleFixture:
    ...
    def install_fake_herdr(self) -> tuple[Path, Path]:
        ...
        fake_herdr.write_text(
            ...
            "if args[:2] == ['pane', 'get']:\n"
            "    target = args[2]\n"
            "    print(json.dumps({'result': {'pane': {'pane_id': target, 'tab_id': os.environ.get('HERDR_MAIN_TAB_ID', 'tab-1'), 'workspace_id': os.environ.get('HERDR_WORKSPACE_ID', 'workspace-1')}}}))\n"
            "elif args[:2] == ['pane', 'layout']:\n"
            "    target = args[args.index('--pane') + 1]\n"
            "    area = {'x': 0, 'y': 0, 'width': 200, 'height': 60}\n"
            "    panes = json.loads(os.environ.get('HERDR_LAYOUT_PANES', json.dumps([{'pane_id': target, 'rect': area}])))\n"
            "    splits = json.loads(os.environ.get('HERDR_LAYOUT_SPLITS', '[]'))\n"
            "    print(json.dumps({'result': {'layout': {'area': area, 'panes': panes, 'splits': splits}}}))\n"
            ...
            "    tab_id = os.environ.get('HERDR_NEW_TAB_ID', 'new-tab')\n"
            "    pane_id = os.environ.get('HERDR_NEW_TAB_PANE_ID', 'new-pane')\n"
            "    if os.environ.get('HERDR_MALFORMED_TAB_CREATE') == 'missing-root':\n"
            "        print(json.dumps({'result': {'tab': {'tab_id': tab_id}}}))\n"
            "    elif os.environ.get('HERDR_MALFORMED_TAB_CREATE') == 'missing-tab':\n"
            "        print(json.dumps({'result': {'root_pane': {'pane_id': pane_id, 'tab_id': tab_id}}}))\n"
            "    else:\n"
            "        print(json.dumps({'result': {'tab': {'tab_id': tab_id}, 'root_pane': {'pane_id': pane_id, 'tab_id': tab_id}}}))\n"
            "elif args[:2] == ['pane', 'list']:\n"
            "    print(json.dumps({'result': {'panes': json.loads(os.environ.get('HERDR_PANE_LIST', '[]'))}}))\n"
            "elif args[:2] == ['pane', 'rename'] and os.environ.get('HERDR_FAIL_PANE_RENAME') == '1':\n"
            "    print(json.dumps({'error': {'code': 'pane_rename_failed', 'message': 'simulated pane rename failure'}}), file=sys.stderr)\n"
            "    raise SystemExit(1)\n"
            "elif args[:2] == ['tab', 'rename'] and os.environ.get('HERDR_FAIL_TAB_RENAME') == '1':\n"
            "    print(json.dumps({'error': {'code': 'tab_rename_failed', 'message': 'simulated tab rename failure'}}), file=sys.stderr)\n"
            "    raise SystemExit(1)\n"
            "elif args[:2] == ['tab', 'close'] and os.environ.get('HERDR_FAIL_TAB_CLOSE') == '1':\n"
            "    print(json.dumps({'error': {'code': 'tab_close_failed', 'message': 'simulated tab close failure'}}), file=sys.stderr)\n"
            "    raise SystemExit(1)\n"
            "elif args[:2] == ['pane', 'close'] and os.environ.get('HERDR_FAIL_PANE_CLOSE') == '1':\n"
            "    print(json.dumps({'error': {'code': 'pane_close_failed', 'message': 'simulated pane close failure'}}), file=sys.stderr)\n"
            "    raise SystemExit(1)\n"
            ...
            "        del agent['name']\n"
            "    sessionless = os.environ.get('HERDR_OMIT_AGENT_SESSION') == '1' or target in json.loads(os.environ.get('HERDR_SESSIONLESS_PANES', '[]'))\n"
            "    if not sessionless and not blocked and (not prompt_positions or get_after_prompt > delay):\n"
            ...
            "    caller_processes = [{'pid': os.getppid()}] if os.environ.get('HERDR_PROCESS_INFO_CALLER') == '1' else []\n"
            "    info = process_infos.get(target, {'pane_id': target, 'foreground_processes': []})\n"
            "    info = {**info, 'foreground_processes': [*info.get('foreground_processes', []), *caller_processes]}\n"
            ...
            "elif args[:2] == ['agent', 'prompt'] and os.environ.get('HERDR_ACCEPT_HANDOFF_SCRIPT'):\n"
            "    import subprocess\n"
            "    path = next(line.split(': ', 1)[1] for line in args[3].splitlines() if line.startswith('Orchestrator handoff file: '))\n"
            "    with open(path) as f:\n"
            "        receiver = json.load(f)['receiver_pane_id']\n"
            "    accepted = subprocess.run([sys.executable, os.environ['HERDR_ACCEPT_HANDOFF_SCRIPT'], '--handoff-path', path, '--owner', 'shipping-task', '--coordination-graph', 'single-loop', '--reality-anchor', 'testing at the executable lifecycle boundary'], env={**os.environ, 'HERDR_PANE_ID': receiver}, capture_output=True, text=True)\n"
            "    with open(os.environ['HERDR_CAPTURE'] + '.accept', 'a') as f:\n"
            "        f.write(accepted.stdout + accepted.stderr)\n"
            "    print(json.dumps({'result': {'agent': {'name': 'orchestrator', 'agent_status': 'working'}}}))\n"
            "elif args[:2] == ['agent', 'prompt'] and os.environ.get('HERDR_AUTO_ACCEPT_HANDOFF') == '1':\n"
            "    path = next(line.split(': ', 1)[1] for line in args[3].splitlines() if line.startswith('Orchestrator handoff file: '))\n"
            "    with open(path) as f:\n"
            "        handoff = json.load(f)\n"
            "    handoff['status'] = 'accepted'\n"
            "    handoff['accepted_by_pane'] = handoff['receiver_pane_id']\n"
            "    handoff['accepted_at'] = '2026-09-02T00:00:00+00:00'\n"
            "    handoff['route'] = {'routed_through': 'boss-say', 'owner': 'shipping-task', 'coordination_graph': 'single-loop', 'reality_anchor': 'testing at the executable lifecycle boundary', 'routed_at': '2026-09-02T00:00:00+00:00'}\n"
