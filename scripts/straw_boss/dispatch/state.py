...
def render_app_hazards_section(app_hazards: AppHazards | None) -> str:
    ...
    for entry in app_hazards.local_files:
        ...
        suffix = f": {entry.risk}" if entry.risk else ""
        lines.append(f"- `{entry.path}`{tag}{suffix}")
    return "\n## App hazards\n\n" + "\n".join(lines) + "\n"
...
- Report progress with:
  `{progress} {path_argument} --note '<summary>' [--ref '<evidence>'] [--waiting-on '<what resumes you>']`
- Reach the main agent with these two commands -- a question for integrated context, and a checkpoint naming who can unblock you, after whose reply you continue instead of replacing it with a terminal status. When resuming from `awaiting-user-input` or `awaiting-authorization`, report `awaiting-main-agent` first, then send the question so the reply has an open checkpoint:
...
- `awaiting-main-agent` means a coordinator action or fact is required; waiting on your own subagent, review, or CI is progress. To end a turn while one of those, or a scheduled wake, will resume you, record the wait with `--waiting-on`; each wait note covers one stop. A reply resolves a checkpoint without changing its status, so report a fresh status or wait note before stopping again.
