...
#### Scenario: Main agent receives an awaiting-main-agent push
- **WHEN** the main agent receives a `SendMessage` push reporting a dispatched agent's `awaiting-main-agent` checkpoint
...
#### Scenario: Fallback detection surfaces an unresolved checkpoint
- **WHEN** the main agent's own fallback detection (e.g. a `Monitor` polling loop) surfaces a task still in `awaiting-main-agent`
