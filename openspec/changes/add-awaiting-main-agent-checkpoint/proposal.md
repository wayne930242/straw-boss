...
The only existing channel for reaching the main agent — the informational-question `SendMessage` — is explicitly fire-and-forget: no status file, no `Monitor` coverage, and it queues behind whatever the main agent is already doing at the same priority as any other async question.
...
- The `Monitor` fallback polling loop's watched-status list gains `awaiting-main-agent`; on it, the main agent itself resolves the checkpoint (not "tell the user which pane to answer in").
...
- `dispatch-completion-reporting`: the `SendMessage` push requirement, the `Monitor`-fallback watched-status list, and the terminal/non-terminal status-file mechanics all gain the new `awaiting-main-agent` checkpoint alongside the existing `done`/`failed`/`awaiting-authorization`/`awaiting-user-input`/`cancelled` set.
...
- `skills/dispatching-work/references/plan-mechanics.md` — new "Main-agent-action checkpoints" section, escalation order, instruction-assembly requirement, `Monitor` watched-status list.
...
Explicitly out of scope for this change (raised during review, deferred): a bounded reminder re-send from the dispatched-agent side while it waits in `awaiting-main-agent`, and fixing the `Monitor` polling loop's per-filename (rather than per-filename-plus-status) dedup, which can cause it to miss a status file's later transition after its first-ever write was already seen. Both are pre-existing gaps in the broader reporting mechanism, not specific to this checkpoint, and are left for a follow-up change.
